...
void main() {
  test('반복 JSON이 정확하고 키가 10진수 순번이다', () {
  ...
  test('변경 영수증이 커밋된 무동작과 버전 증가를 구분한다', () {
  ...
  test('주간 요일 선택이 안정적인 전역 순번을 사용한다', () {
    final event = PlannerEvent(
      id: 'weekly',
      groupId: 'g',
      title: 'weekly',
  ...
  test('월간 날짜가 2월과 짧은 달의 마지막 날로 제한된다', () {
    final event = PlannerEvent(
      id: 'monthly',
      groupId: 'g',
      title: 'monthly',
  ...
  test('월간 순번 0은 기준 시각 이후의 첫 유효한 날짜를 사용한다', () {
    final event = PlannerEvent(
      id: 'monthly-after-anchor',
      groupId: 'g',
      title: 'monthly after anchor',
      startAt: DateTime.utc(2030, 1, 20, 9),
      endAt: DateTime.utc(2030, 1, 20, 10),
      ownerId: 'demo-user',
      timezone: 'UTC',
      recurrenceRule: RecurrenceRule(
        frequency: RecurrenceFrequency.monthly,
    ...
    final first = recurringOccurrenceAtIndex(event, 0);
    expect(first, isNotNull);
    expect(first!.occurrenceIndex, 0);
    expect(first.startAt, DateTime.utc(2030, 2, 15, 9));

    final rows = expandRecurringEvent(
      event,
      _range(DateTime.utc(2030, 1, 1), DateTime.utc(2030, 8, 1)),
    );
    expect(rows.map((row) => row.occurrenceIndex), <int>[0, 1, 2]);
    expect(rows.map((row) => row.startAt), <DateTime>[
      DateTime.utc(2030, 2, 15, 9),
      DateTime.utc(2030, 4, 15, 9),
      DateTime.utc(2030, 6, 15, 9),
    ]);
  });

  test('월간 종료일이 정규화된 순번 0보다 이르면 쓰기 전에 거부한다', () async {
    final repository = LocalScheduleRepository();
    final draft = EventDraft(
      title: 'until before monthly occurrence zero',
      startAt: DateTime.utc(2030, 1, 20, 9),
      endAt: DateTime.utc(2030, 1, 20, 10),
      timezone: 'UTC',
      recurrence: RecurrenceRule(
        ...
        untilDate: DateTime(2030, 1, 20),
      ),
    );

    await expectLater(
      repository.createRecurringEvent('demo-user', 'demo-group', draft),
      throwsA(
    ...
    final page = await repository.eventsForRange(
      userId: 'demo-user',
      groupId: 'demo-group',
      range: _range(DateTime.utc(2030, 1, 1), DateTime.utc(2030, 4, 1)),
    );
    expect(page.events.where((event) => event.title == draft.title), isEmpty);

    final boundaryDraft = draft.copyWith(
      title: 'until at monthly occurrence zero',
      recurrence: RecurrenceRule(
        frequency: RecurrenceFrequency.monthly,
        interval: 2,
        monthlyDay: 15,
    ...
    expect(created.startAt, DateTime.utc(2030, 2, 15, 9));
  });

  test('로컬 반복 범위 페이지 처리 및 이번/이후/전체 변경', () async {
    final repository = LocalScheduleRepository(
  ...
  test('로컬 전체 범위가 fan-out 없이 단일 일정을 반복 일정으로 왕복 변환한다', () async {
    final repository = LocalScheduleRepository();
  ...
  test('로컬 이후 분할이 바로 앞 구간을 닫는다', () async {
    final repository = LocalScheduleRepository();
    final anchor = await repository.createRecurringEvent(
      'demo-user',
      'demo-group',
  ...
  test('로컬 반복 구간과 재정의가 시리즈별로 격리된다', () async {
    final repository = LocalScheduleRepository();
  ...
  test('로컬 이후 편집이 활성 구간 규칙을 상속한다', () async {
    final repository = LocalScheduleRepository();
    final anchor = await repository.createRecurringEvent(
      'demo-user',
      'demo-group',
  ...
  test('종일 반복 행이 현지 반개구간 날짜 경계를 사용한다', () async {
    final event = PlannerEvent(
      id: 'all-day',
      groupId: 'g',
      title: 'all-day',
  ...
  test('현지 시각 변환이 DST 공백은 앞으로, 중첩은 표준시로 해석한다', () {
    final gap = wallTimeToUtc(DateTime(2026, 3, 8, 2, 30), 'America/New_York');
  ...
  test('로컬 규칙 기간이 양방향 DST 전환에서 현지 달력 날짜를 사용한다', () async {
    final rule = RecurrenceRule(
      frequency: RecurrenceFrequency.daily,
      end: RecurrenceEnd.never,
    );
    ...
    expect(fallEnd.difference(fallStart), const Duration(days: 366, hours: 1));
  ...
  test('Supabase v2 행 파서가 완전한 발생 일정 투영값을 유지한다', () async {
    ...
    final repository = _AuthenticatedSupabaseScheduleRepository(client, 'hint');
    final page = await repository.eventsForRange(
      userId: 'hint',
      groupId: 'group-1',
  ...
  test('발생 순번과 v2 커서가 부호 있는 bigint 범위 안에 있다', () {
  ...
  test('로컬 단건 조회가 유효하지만 구체화할 수 없는 순번을 누락으로 처리한다', () async {
    final repository = LocalScheduleRepository();
  ...
  test('현지 달력 반복 계산이 DST 전환에도 안정적이다', () {
  ...
  test('자정을 넘는 시간 지정 발생이 하루의 현지 이전 조회 범위를 사용한다', () {
  ...
  test('순번 0에서 로컬 이후 삭제가 기준 일정에 삭제 표식을 남긴다', () async {
    final repository = LocalScheduleRepository();
    final anchor = await repository.createRecurringEvent(
      'demo-user',
      'demo-group',
  ...
  test('로컬에서 범위 안으로 이동한 재정의가 한 번 포함되고 유효 시각순으로 정렬된다', () async {
    final repository = LocalScheduleRepository();
    final anchor = await repository.createRecurringEvent(
      'demo-user',
      'demo-group',
    ...
    expect(rows.map((row) => row.identityKey).toSet(), hasLength(rows.length));
  ...
  test('로컬 반복 멤버 변경이 전체 재정의 스냅샷을 갱신한다', () async {
    final repository = LocalScheduleRepository();
    final rule = RecurrenceRule(
      frequency: RecurrenceFrequency.daily,
      end: RecurrenceEnd.count,
    ...
    final overridden = rows.singleWhere((event) => event.occurrenceIndex == 1);
    await repository.updateEventOccurrence(
      event: overridden,
      draft: EventDraft(
  ...
  test('컨트롤러 기본 상세 조회가 반복 기준 일정을 순번 0으로 별칭 처리한다', () async {
    final repository = LocalScheduleRepository();
    final anchor = await repository.createRecurringEvent(
      'demo-user',
      'demo-group',
  ...
  test('로컬 반복 참여자 교체가 생성자를 유지하고 정확한 영수증을 반환한다', () async {
    final repository = LocalScheduleRepository();
    final rule = RecurrenceRule(
      frequency: RecurrenceFrequency.daily,
      end: RecurrenceEnd.count,
  ...
  test('Supabase v2가 짧은 has_more 페이지와 일관되지 않은 구체화 행을 거부한다', () async {
    final row = <String, dynamic>{
      'id': 'event-1',
      'event_id': 'event-1',
      'series_id': 'event-1',
  ...
  test('Supabase 반복 생성이 정규 첫 발생 일정 행만 허용한다', () async {
    final rule = RecurrenceRule(
      frequency: RecurrenceFrequency.daily,
      end: RecurrenceEnd.count,
      count: 2,
  ...
  test('Supabase 반복 생성이 규칙에서 도출한 순번 0 구체화를 검증한다', () async {
    ...
    PlannerEvent seriesFor(String id, EventDraft draft, RecurrenceRule rule) =>
        PlannerEvent(
          id: id,
          groupId: 'group-1',
          title: draft.title,
    ...
    final monthlySeries = seriesFor('event-monthly', monthlyDraft, monthlyRule);
    ...
    final afterAnchorRule = RecurrenceRule(
      frequency: RecurrenceFrequency.monthly,
      interval: 2,
      monthlyDay: 15,
      end: RecurrenceEnd.count,
    ...
    final afterAnchorDraft = EventDraft(
      title: 'monthly after anchor',
      startAt: DateTime.utc(2030, 1, 20, 9),
      endAt: DateTime.utc(2030, 1, 20, 10),
      timezone: 'UTC',
      memberIds: const <String>['demo-user'],
      recurrence: afterAnchorRule,
    );
    final afterAnchorSeries = seriesFor(
      'event-monthly-after-anchor',
      afterAnchorDraft,
      afterAnchorRule,
    );
    final afterAnchorO0 = recurringOccurrenceAtIndex(afterAnchorSeries, 0)!;
    expect(afterAnchorO0.startAt, DateTime.utc(2030, 2, 15, 9));
    final afterAnchorClient = _client(
      rowFor(
        id: 'event-monthly-after-anchor',
        draft: afterAnchorDraft,
        rule: afterAnchorRule,
        occurrence: afterAnchorO0,
      ),
    );
    addTearDown(afterAnchorClient.dispose);
    final afterAnchorRepository = _AuthenticatedSupabaseScheduleRepository(
      afterAnchorClient,
      'demo-user',
    );
    final afterAnchorCreated = await afterAnchorRepository.createRecurringEvent(
      'demo-user',
      'group-1',
      afterAnchorDraft,
    );
    expect(afterAnchorCreated.occurrenceIndex, 0);
    expect(afterAnchorCreated.startAt, DateTime.utc(2030, 2, 15, 9));
    ...
    // 기준일(15)은 monthly_day(31)와 다르고, 구체화된 날짜는 이 시간대의
    // 봄철 DST 경계를 지난다.
  ...
  test('Supabase v2가 시간대 없는 수명 주기 타임스탬프를 거부한다', () async {
    final row = <String, dynamic>{
      'id': 'event-lifecycle',
