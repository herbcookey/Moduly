// 범위가 제한된 달력 읽기와 로컬 달력 투영에 집중한 비 UI 검사다. Supabase
// 마이그레이션에는 독립 SQL 테스트가 있으며, 아래 원격 어댑터 사례는 요청과
// 파서의 경계만 검사한다.
...
/// 원격 어댑터는 v2 범위 RPC를 호출하기 전에 의도적으로 활성 인증 컨텍스트를
/// 요구한다. 전송 계층 테스트는 JWT를 만드는 대신 명시적인 이음새를 통해 해당
/// 컨텍스트를 제공한다.
...
void main() {
  ...
  group('범위 제한 달력 모델 및 시간대 정책', () {
    test('커서가 엄격한 v1 URL 안전 base64이며 페이지 스냅샷이 불변이다', () {
    ...
    test('일간/월간/일정 목록 경계가 현지 자정과 DST에 안전한 날짜 수를 사용한다', () {
      ...
      expect(
        calendarDateSpan(february.startUtc, february.endUtc, february.timezone),
        35,
      );
    ...
    test('시간 지정 겹침은 시각 반개구간이고 종일 겹침은 날짜 반개구간이다', () {
  ...
  group('LocalScheduleRepository 범위 제한 읽기', () {
    test('정확한 겹침, 참여자 멤버십, 1,000행 이상의 키셋 페이지를 사용한다', () async {
  ...
  group('Supabase 범위 제한 RPC 경계', () {
    test('현재 세션이 없으면 v2 범위 및 단건 읽기가 안전하게 실패한다', () async {
      ...
      final repository = _AuthenticatedSupabaseScheduleRepository(client, null);
    ...
    test('정확한 범위 매개변수를 직렬화하고 봉투를 엄격하게 파싱한다', () async {
    ...
    test('정규화됐거나 비정규인 시간 지정 타임스탬프를 거부한다', () async {
      final payload = <String, dynamic>{
        'events': <Object>[_eventRow()],
        'next_cursor': null,
        'has_more': false,
  ...
  test('기존 Realtime 병합이 잘못된 행을 거부하고 엄격한 오프셋을 허용한다', () async {
  ...
  test('기존 로컬 하위 클래스는 생성자가 생략된 응답을 정규화할 수 있지만 엄격 모드는 거부한다', () async {
  ...
  group('PlannerController 범위 제한 상태', () {
    ...
    test('늦은 첫 페이지 결과가 최신 범위를 덮어쓸 수 없다', () async {
    ...
    test('사용자 및 그룹 변경이 늦은 첫 페이지 응답을 차단한다', () async {
      final first = Completer<EventRangePage>();
      final second = Completer<EventRangePage>();
      final repository = _RangeRepository(<EventRangePage>[]);
      final pending = <Completer<EventRangePage>>[first, second];
      ...
      await Future<void>.delayed(const Duration(milliseconds: 1));
      expect(controller.events.map((event) => event.id), <String>['group-two']);

      // 선택한 그룹과 범위가 우연히 그대로여도 사용자 변경은 독립적인
      // 개인정보 보호 경계다.
    ...
    test('참여자 필터가 오래된 페이지를 차단하고 새 필터를 전송한다', () async {
      final first = Completer<EventRangePage>();
      final second = Completer<EventRangePage>();
      final repository = _RangeRepository(<EventRangePage>[]);
      final pending = <Completer<EventRangePage>>[first, second];
      ...
      await Future<void>.delayed(const Duration(milliseconds: 1));
      expect(controller.events.map((event) => event.id), <String>['filtered']);
      expect(controller.selectedMemberId, 'member-jin');
    });

    test('더 불러오기가 직렬화되고 오래된 페이지 처리가 새 범위를 바꿀 수 없다', () async {
      final initial = Completer<EventRangePage>();
      final loadMore = Completer<EventRangePage>();
      final nextRange = Completer<EventRangePage>();
      final repository = _RangeRepository(<EventRangePage>[]);
      final pending = <Completer<EventRangePage>>[initial, loadMore, nextRange];
      repository.responseForRange = (_) => pending.removeAt(0).future;
      final controller = PlannerController(
        auth: AuthRepository(),
        repository: repository,
      ...
      await Future<void>.delayed(const Duration(milliseconds: 2));
      expect(controller.events.map((event) => event.id), <String>['new-range']);
    ...
    test('디바운스된 무효화가 한 번 다시 조회하고 새로 고침 중에는 대기한다', () async {
      final repository = _RangeRepository(<EventRangePage>[]);
      var responseCount = 0;
    ...
    test('권위 있는 범위 거부가 마지막 정상 비공개 스냅샷을 지운다', () async {
      final repository = _RangeRepository(<EventRangePage>[]);
      var responses = 0;
      repository.responseForRange = (range) {
        responses++;
    ...
    test('일시적 범위 새로 고침 실패가 마지막 정상 스냅샷을 보존한다', () async {
      final repository = _RangeRepository(<EventRangePage>[]);
      var responses = 0;
      repository.responseForRange = (range) {
        responses++;
    ...
    test('더 불러오기가 동일 페이로드의 무효화 중복을 멱등하게 병합한다', () async {
      final repository = _RangeRepository(<EventRangePage>[]);
      var responses = 0;
      repository.responseForRange = (range) {
        responses++;
      ...
      // 이 불변 페이로드를 이미 포함한 페이지보다 부모 무효화/upsert가 먼저
      // 도착하는 상황을 재현한다.
    ...
    test('범위 제한 컨트롤러가 요청 한도보다 큰 응답을 거부한다', () async {
      final repository = _RangeRepository(<EventRangePage>[]);
      repository.responseForRange = (range) {
    ...
    test('기존 refreshSelectedEventRange가 선택 그룹 새로 고침 경로를 다시 시작한다', () async {
    ...
    test('상세 조회가 페이지를 섞지 않고 선택 범위 밖의 일정을 불러온다', () async {
      final repository = _RangeRepository(<EventRangePage>[]);
      final detail = _event(
        id: 'outside-range',
        groupId: 'group-1',
    ...
    test('그룹 전환 후 늦게 온 상세 조회 결과를 버린다', () async {
